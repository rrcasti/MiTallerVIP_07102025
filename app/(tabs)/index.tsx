...
// 👇 SUGERENCIA: Importa el hook 'useAuth' que ya usas en otras pantallas
...
export default function DashboardScreen() {
    ...
    // 👇 SUGERENCIA: Usa el hook 'useAuth' para obtener el usuario. Es más robusto que 'auth.currentUser'.
    ...
    // 👇 SUGERENCIA: Se refactoriza el useEffect para ser más claro y usar el hook 'useAuth'.
    // Esta lógica es más segura y evita recargas innecesarias.
    const loadDashboardData = useCallback(async () => {
        if (!user) return;

        setLoadingData(true);
        try {
            const [userVehicles, repairsData] = await Promise.all([
                getVehiclesForUser(user.uid),
                getRepairsForUser(user.uid),
            ]);
    ...
    // El return principal no tiene cambios estructurales, solo se eliminó cualquier posible
    // espacio en blanco o carácter oculto que pudiera estar causando el crash.
    return (
        <SafeAreaView style={styles.safeArea}>
